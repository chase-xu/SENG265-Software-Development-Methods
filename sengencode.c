...
{
	/****** To be completed ******/
	int i = 0, index = 0, cnt = 0, j = 0;
	for (; i < size; i++) {
		if (code[i] != 0)msg[index++] = code[i];
		else {
			if (i + 1 < size) {
				cnt = code[i + 1];
				for (j = 0; j < cnt; j++) {
		...
		if (msg[i] != 1) {
			...
			if (i >= size) {
				if (cnt >= 3) {
					...
					while (beg < size) code[index++] = msg[beg++];
				}
			}
			else {
				if (cnt >= 3) {
					...
					while (beg < i) code[index++] = msg[beg++];
				}
			}
		}
	}
...
{
	/****** To be completed ******/
	int codeTmp[MAX_MSG_SIZE] = { 0 }, i = 0;
	/* You can take the check_mtf() function as an example*/
	int cnt = run_length_decode(code, size, codeTmp);
	for (i = 0; i < cnt; i++) {
		if (msg[i] != codeTmp[i]) return 0;
	}
	return 1;
}
...
{
	char *msg2 = (char *)malloc(sizeof(char) * size + 1); /*I have added + 1 becuase of "\0"
...
{
	char msg_array[3][MAX_MSG_SIZE] = { "abcabcaaaaaab","abbbbbcccccdecccceee","abbbbbcccccdecccceee" };
...
void run_length_test(void)
{
	...
	for (i = 0; i < 5; i++)
	{
		...
		int ss = run_length_decode(rl_code, new_len, mtf_code);
